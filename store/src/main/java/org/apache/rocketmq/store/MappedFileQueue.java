...
public class MappedFileQueue implements Swappable {
  ...
  public MappedFile getConsumeQueueMappedFileByTime(final long timestamp,
      CommitLog commitLog,
  ...
  public MappedFile getLastMappedFile(final long startOffset,
    ...
    // 如果当前映射文件队列为空，即没有最后一个映射文件
    if (mappedFileLast == null) {
      // 计算新的映射文件的起始偏移量，确保它是映射文件大小的倍数
      createOffset = startOffset - (startOffset % this.mappedFileSize);
    }

    // 如果最后一个映射文件已满
    if (mappedFileLast != null && mappedFileLast.isFull()) {
      // 计算新的映射文件的起始偏移量，为当前最后一个映射文件的结束偏移量
      createOffset = mappedFileLast.getFileFromOffset() + this.mappedFileSize;
    }

    // 如果需要创建新的映射文件且计算出了新的起始偏移量
  ...
  public MappedFile tryCreateMappedFile(long createOffset) {
    ...
    // 调用 doCreateMappedFile 方法创建新的映射文件
  ...
  public int deleteExpiredFileByOffsetForTimerLog(long offset, int checkOffset,
      int unitSize) {
    Object[] mfs = this.copyMappedFiles(0);
