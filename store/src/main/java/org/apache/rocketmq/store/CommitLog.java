...
public class CommitLog implements Swappable {
  ...
  public boolean getData(final long offset, final int size,
  ...
  public List<SelectMappedBufferResult> getBulkData(final long offset,
      final int size) {
  ...
  public void recoverNormally(
      long maxPhyOffsetOfConsumeQueue) throws RocksDBException {
  ...
  private void setBatchSizeIfNeeded(Map<String, String> propertiesMap,
   ...
   * 在提交日志附加消息后调用的方法。
   *
   * @param msg           消息对象，包含消息的详细信息
   * @param result        消息附加结果，包含附加操作的状态和其他相关信息
   * @param commitLogFile 当前的提交日志文件，用于记录消息的存储位置
   */
  protected void onCommitLogAppend(MessageExtBrokerInner msg,
      AppendMessageResult result, MappedFile commitLogFile) {
    // 调用消息存储对象的方法，处理提交日志的附加操作
    // 这个方法通常用于执行一些后续的处理逻辑，例如更新索引、记录日志等
    this.getMessageStore().onCommitLogAppend(msg, result, commitLogFile);
  }


  private boolean isMappedFileMatchedRecover(
      final MappedFile mappedFile) throws RocksDBException {
    ByteBuffer byteBuffer = mappedFile.sliceByteBuffer();
  ...
  public void updateMaxMessageSize(
  ...
  public CompletableFuture<PutMessageResult> asyncPutMessage(
    ...
    // 初始化需要解锁的MappedFile为null
    MappedFile unlockMappedFile = null;
    // 获取当前队列中的最后一个MappedFile
    MappedFile mappedFile = this.mappedFileQueue.getLastMappedFile();

    // 当前偏移量
    long currOffset;
    if (mappedFile == null) {
      currOffset = 0; // 如果最后一个MappedFile不存在，则当前偏移量为0
    } else {
      currOffset = mappedFile.getFileFromOffset() + mappedFile.getWrotePosition(); // 否则，计算当前偏移量
    }
    ...
    // 如果锁定时间超过500毫秒，记录警告日志
    if (elapsedTimeInLock > 500) {
      log.warn("[NOTIFYME]putMessage in lock cost time(ms)={}, bodyLength={} AppendMessageResult={}", elapsedTimeInLock, msg.getBody().length, result);
    }

    // 如果需要解锁MappedFile且启用了预热MappedFile功能
  ...
  private CompletableFuture<PutMessageStatus> handleDiskFlush(
  ...
  private CompletableFuture<PutMessageStatus> handleHA(
      AppendMessageResult result, PutMessageResult putMessageResult,
      int needAckNums) {
  ...
  class DefaultAppendMessageCallback implements AppendMessageCallback {
    ...
    public AppendMessageResult handlePropertiesForLmqMsg(
        ByteBuffer preEncodeBuffer,
     ...
     * @param byteBuffer        字节缓冲区，用于读取和写入数据
     * @param maxBlank          最大空白区域
     * @param msgInner          消息对象，包含消息的详细信息
     * @param putMessageContext 消息上下文，包含消息写入时的额外信息
     * @return 消息追加的结果，包含写入状态和相关信息
     */
    public AppendMessageResult doAppend(final long fileFromOffset,
      ...
      if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {
        ...
        // 3 剩余空间可以是任意值
        // 这里特别设置了 maxBlank 的长度
        final long beginTimeMills = CommitLog.this.defaultMessageStore.now();
        byteBuffer.put(this.msgStoreItemMemory.array(), 0, 8);
        return new AppendMessageResult(AppendMessageStatus.END_OF_FILE, wroteOffset,
            maxBlank, /* 只写了 8 字节，但声明写了 maxBlank 以计算写入位置 */
  ...
  class DefaultFlushManager implements FlushManager {
    ...
    @Override
    public CompletableFuture<PutMessageStatus> handleDiskFlush(
        AppendMessageResult result, MessageExt messageExt) {
      // Synchronization flush
      if (FlushDiskType.SYNC_FLUSH == CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
        final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;
  ...
  @Override
  public void swapMap(int reserveNum, long forceSwapIntervalMs,
  ...
  public class ColdDataCheckService extends ServiceThread {
     ...
     */
    public boolean isMsgInColdArea(String group, String topic, int queueId,
        long offset) {
      if (!defaultMessageStore.getMessageStoreConfig().isColdDataFlowControlEnable()) {
        return false;
      }
