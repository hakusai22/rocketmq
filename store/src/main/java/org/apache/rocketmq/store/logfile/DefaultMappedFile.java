...
public class DefaultMappedFile extends AbstractMappedFile {
  ...
  public DefaultMappedFile(final String fileName,
  ...
  public AppendMessageResult appendMessage(final ByteBuffer byteBufferMsg,
      final CompactionAppendMsgCallback cb) {
  ...
  public AppendMessageResult appendMessages(
      final MessageExtBatch messageExtBatch, final AppendMessageCallback cb,
      PutMessageContext putMessageContext) {
   ...
   * 将消息追加到当前映射文件中。
   *
   * @param messageExt        消息对象，包含待写入的消息数据
   * @param cb                回调对象，用于处理消息写入的具体逻辑
  ...
  public AppendMessageResult appendMessagesInner(final MessageExt messageExt,
    ...
    // 如果当前已写入的位置大于等于文件大小，记录错误日志并返回未知错误
    log.error("MappedFile.appendMessage return null, wrotePosition: {} fileSize: {}", currentPos, this.fileSize);
    return new AppendMessageResult(AppendMessageStatus.UNKNOWN_ERROR);
  }
   ...
   */
  @Override
  public boolean appendMessage(final byte[] data, final int offset,
      final int length) {
    int currentPos = WROTE_POSITION_UPDATER.get(this);
  ...
  private class Itr implements Iterator<SelectMappedBufferResult> {
    ...
    public void forEachRemaining(
        Consumer<? super SelectMappedBufferResult> action) {
