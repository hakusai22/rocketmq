...
public abstract class NettyRemotingAbstract {
   ...
   */
  public NettyRemotingAbstract(final int permitsOneway,
      final int permitsAsync) {
  ...
  public void doAfterRpcHooks(String addr, RemotingCommand request,
  ...
  public static void writeResponse(Channel channel, RemotingCommand request,
      @Nullable RemotingCommand response) {
   ...
   * 同步调用实现方法。
   *
   * @param channel       用于通信的Netty通道
   * @param request       发送的请求命令对象
   * @param timeoutMillis 请求的超时时间（毫秒）
   * @return 从远程服务器接收到的响应命令对象
   * @throws InterruptedException         如果线程被中断
   * @throws RemotingSendRequestException 如果发送请求时发生异常
   * @throws RemotingTimeoutException     如果等待响应时发生超时
   */
  public RemotingCommand invokeSyncImpl(final Channel channel,
    ...
    try {
      // 调用 `invokeImpl` 方法发送请求并获取响应的未来对象
      return invokeImpl(channel, request, timeoutMillis)
          .thenApply(ResponseFuture::getResponseCommand)  // 将未来对象转换为响应命令对象
          .get(timeoutMillis, TimeUnit.MILLISECONDS);  // 等待响应命令对象，最大等待时间为 `timeoutMillis`
    } catch (ExecutionException e) {
      // 如果执行过程中发生异常，捕获 `ExecutionException` 并抛出 `RemotingSendRequestException`
      throw new RemotingSendRequestException(channel.remoteAddress().toString(), e.getCause());
    } catch (TimeoutException e) {
      // 如果等待响应时发生超时，捕获 `TimeoutException` 并抛出 `RemotingTimeoutException`
   ...
   * @param request       发送的请求命令对象
   * @param timeoutMillis 请求的超时时间（毫秒）
   * @return 一个表示异步操作结果的CompletableFuture
   */
  protected CompletableFuture<ResponseFuture> invoke0(final Channel channel,
    ...
    if (acquired) {
      ...
      try {
        ...
        // 如果发送请求时发生异常，移除响应表中的条目，释放资源，并完成future
        responseTable.remove(opaque);
        responseFuture.release();
        log.warn("send a request command to channel <{}> channelId={} Exception",
            RemotingHelper.parseChannelRemoteAddr(channel), channel.id(), e);
